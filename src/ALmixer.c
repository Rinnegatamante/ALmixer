...
{
#define INTERNAL_MAX_NUM_EXT_CHARS 8
	const Sound_DecoderInfo **rc = Sound_AvailableDecoders();
	char lower_case_usr_ext[32];
	char lower_case_info_ext[32];
	size_t usr_ext_strlen = 0;
	
	if(NULL == usr_ext)
	{
		return AL_FALSE;
	}
	usr_ext_strlen = strlen(usr_ext);

	if(usr_ext_strlen < 1)
	{
		return AL_FALSE;
	}
	if(usr_ext_strlen >= INTERNAL_MAX_NUM_EXT_CHARS)
	{
		return AL_FALSE;
	}
	...
	{
		size_t j = 0;
		for(j=0; (j<usr_ext_strlen && j<INTERNAL_MAX_NUM_EXT_CHARS); j++)
		{
			lower_case_usr_ext[j] = tolower(usr_ext[j]);
		}
		lower_case_usr_ext[j] = '\0';
	}
...

	{
		...
		for(i = rc; *i != NULL; i++)
		{
			...
			for(ext = (*i)->extensions; *ext != NULL; ext++)
			{
				size_t info_ext_strlen = strlen(*ext);
				//fprintf(stderr, "   File extension \"%s\"\n", *ext);

				if(info_ext_strlen != usr_ext_strlen)
				{
					continue;
				}
				else
				{
					...
					size_t j = 0;
					for(j=0; (j<info_ext_strlen && j<INTERNAL_MAX_NUM_EXT_CHARS); j++)
					{
						lower_case_info_ext[j] = tolower((*ext)[j]);
					}
					lower_case_info_ext[j] = '\0';
				
					if(0 == strcmp(lower_case_usr_ext, lower_case_info_ext))
					{
						return AL_TRUE;
					}
				}
...
#undef INTERNAL_MAX_NUM_EXT_CHARS
...
static ALmixer_Data* DoLoad(Sound_Sample* sample, ALuint buffersize, ALboolean decode_mode_is_predecoded, ALuint max_queue_buffers, ALuint num_startup_buffers, ALuint suggested_number_of_buffers_to_queue_per_update_pass, ALuint access_data)
{
	...
	if(AL_FALSE == decode_mode_is_predecoded)
	{
		...
		/* With the Ogg decoder (both the SDL_sound and the new one, because I wrote the the fill buffer routines for both),
		   we will not get an EOF immediately.
		   This is because Ogg tends to read in tiny packets smaller than the requested buffer.
		   So the decoder keeps reading until its full.
		   However, there is a case where the buffer does not get completely filled because the remaining space is smaller than the the minimum packet read size.
		   So the decoder returns EGAIN instead of EOF so the next time there is a chance that there is actually more data.
		   So this means we can give up and say Ogg with LoadStream can never "get lucky".
		   Or we need to read the next buffer to see if there is an EOF.
		   The problem is if there is not an EOF, we need to use that data.
		   I originally used to rewind and throw away the data, but rewinding was not working correctly with the Android OpenSL ES decoder.
		   I've been having problems with this algorithm (bugs), so I'm tempted to ditch it.
		   But here's one last attempt.
		 */
		/* Update: I used to do this only for EAGAIN, but I think now I always need to do it. I was missing stuff in my mp3 test because EAGAIN was never hit. */
		...
		{
			ALbyte* temp_read_combined_or_final_buffer = NULL;
			ALuint temp_read_combined_or_final_buffer_num_bytes = 0;
			/*
	fprintf(stderr, "We got LUCKY! File is predecoded even though STREAM was requested\n");
*/
			/* So it is possible for the non-Ogg decoders that we got EOF in the second read.
			   That means we might have data in the second buffer.
			*/
...
//			temp_read_buffer2 = NULL;
			 ...
			 */
			ret_data->total_bytes = temp_read_buffer1_num_bytes + temp_read_buffer2_num_bytes;
			/* For now, the loaded bytes is the same as total bytes, but
			 * this could change during a seek operation
			 */
			ret_data->loaded_bytes = temp_read_buffer1_num_bytes + temp_read_buffer2_num_bytes;

			/* Let's compute the total playing time 
			 * SDL_sound does not yet provide this (we're working on
			 * that at the moment...)
			 */
			ret_data->total_time = Compute_Total_Time(&sample->desired, temp_read_buffer1_num_bytes+temp_read_buffer2_num_bytes);
			...
			/* If we have data in the second buffer, we need to combine the two buffers so we can submit it.
		   	*/
			if(temp_read_buffer2_num_bytes > 0)
			{
				temp_read_combined_or_final_buffer_num_bytes = temp_read_buffer1_num_bytes + temp_read_buffer2_num_bytes;
				temp_read_combined_or_final_buffer = (ALbyte*)malloc( sizeof(ALbyte) * temp_read_combined_or_final_buffer_num_bytes);
				if(NULL == temp_read_buffer1)
				{
					ALmixer_SetError("alGenBuffers failed: %s\n", alGetString(error));
					free(temp_read_buffer2);
				...
				/* copy buffer 1 */
				memcpy(temp_read_combined_or_final_buffer, temp_read_buffer1, temp_read_buffer1_num_bytes);
				/* copy buffer 2 at the end of the first copy */
				memcpy(&temp_read_combined_or_final_buffer[temp_read_buffer1_num_bytes], temp_read_buffer2, temp_read_buffer2_num_bytes);
			}
			else
			{
				/* Otherwise, just use the first buffer since it has all the data. */
				temp_read_combined_or_final_buffer = temp_read_buffer1;
				temp_read_combined_or_final_buffer_num_bytes = temp_read_buffer1_num_bytes;
			}
			...
			alBufferData(ret_data->buffer[0], 
				...
				temp_read_combined_or_final_buffer,
				temp_read_combined_or_final_buffer_num_bytes,
				sample->desired.rate
			);
			if( (error = alGetError()) != AL_NO_ERROR)
			{
				ALmixer_SetError("alBufferData failed: %s\n", alGetString(error));
				free(temp_read_combined_or_final_buffer);
				free(temp_read_buffer2);
				free(temp_read_buffer1);
				Sound_FreeSample(sample);
			...
			else
			{
				...
				/* drat: if we have have data in the second buffer, we need to resize the sample->buffer and update associated values. */
				if(temp_read_buffer2_num_bytes > 0)
				{
					free(sample->buffer);
					sample->buffer = malloc(temp_read_combined_or_final_buffer_num_bytes*sizeof(ALbyte));
					if(NULL == sample->buffer)
					{
						free(temp_read_combined_or_final_buffer);
						free(temp_read_buffer2);
						free(temp_read_buffer1);
					...
					memcpy(sample->buffer, temp_read_combined_or_final_buffer, temp_read_combined_or_final_buffer_num_bytes);
					sample->buffer_size = temp_read_combined_or_final_buffer_num_bytes;
				}
				else
				{
					/* easy case where we don't have to resize anything */
					memcpy(sample->buffer, temp_read_buffer1, temp_read_buffer1_num_bytes);
				}
			}
			/* Free the temp buffers now that we are done with it. */
			/* Only free the combined buffer if we actually allocated memory for it. In the other case, it points to the first buffer. */
			if(temp_read_buffer2_num_bytes > 0)
			{
				free(temp_read_combined_or_final_buffer);
				temp_read_combined_or_final_buffer = NULL;
			}
			free(temp_read_buffer2);
			temp_read_buffer2 = NULL;
			free(temp_read_buffer1);
	...
	*/
			...
			*/
			...
			else /* access_data == false */
			{
				// In this case, we already copied the temp buffers into alBuffers and since we are not access_data, we need to free the buffers.
				free(temp_read_buffer2);
				free(temp_read_buffer1);
				temp_read_buffer2 = NULL;
				temp_read_buffer1 = NULL;
...
/* These are the former macro functions, now full blown API functions. */
...
/* End: These are the former macro functions, now full blown API functions. */
