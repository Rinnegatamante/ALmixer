...
void prefetchEventCallback(SLPrefetchStatusItf caller, void *context, SLuint32 event)
{
    ...
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to get FiilLevel from SLPrefetchStatusItf");
        return;
    }
    ...
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to get PrefetchStatus from SLPrefetchStatusItf");
        return;
    }

//    SNDDBG2("prefetch level=%d status=0x%x event=%d", level, status, event);
    SLuint32 new_prefetch_status;
    if ((PREFETCHEVENT_ERROR_CANDIDATE == (event & PREFETCHEVENT_ERROR_CANDIDATE))
            && (level == 0) && (status == SL_PREFETCHSTATUS_UNDERFLOW)) {
        SNDDBG2("PrefetchEventCallback: Error while prefetching data, exiting");
...
//        SNDDBG2("Sound metadata: rate=%d, channels=%d", sample->actual.rate, sample->actual.channels);
    ...
    if (file_container->eos == SL_BOOLEAN_FALSE) {
        ...
        if (SL_RESULT_SUCCESS != result) {
            SNDDBG2("Failed to enqueue to decode buffer queue");
        }
...
static void decodeProgressCallback(
    ...
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to get position from OpenSLES progress callback");
        SignalEos(file_container);
        return;
    }

    if (SL_PLAYEVENT_HEADATEND & event) {
//        SNDDBG2("SL_PLAYEVENT_HEADATEND current position=%u ms", msec);
        SignalEos(file_container);
        return;
    }
}
...
static int OpenSLES_init() {
    ...
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to create OpenSLES engine object");
        return(0);
    } else {
//        SNDDBG2("Create OpenSLES engine object - OK");
    }

    result = (*slEngine)->Realize(slEngine, SL_BOOLEAN_FALSE);
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to realize OpenSLES engine object");
        return(0);
    } else {
//        SNDDBG2("Realize OpenSLES engine object - OK");
    }

    /* Get the SL Engine Interface which is implicit */
    result = (*slEngine)->GetInterface(slEngine, SL_IID_ENGINE, (void*)&slEngineItf);
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to get OpenSLES engine interface");
        return(0);
    } else {
//        SNDDBG2("Get OpenSLES engine interface - OK");
    }
...
static void OpenSLES_quit() {
    if (slEngine != NULL) {
//        SNDDBG2("Destroy OpenSLES Engine");
...
static int OpenSLES_open(Sound_Sample *sample, const char *ext) {
    ...
    if (strncmp(internal->optional_file_name, "/", 1) == 0) {
        ...
        if (fd < 0) {
            SNDDBG2("AAsset_openFileDescriptor failed");
            return(0);
        }
	...
	*/
    ...
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to create OpenSLES audio player");
        return(0);
    } else {
//        SNDDBG2("Create OpenSLES audio player - OK");
    }

    /* Realize the player in synchronous mode. */
    result = (*player)->Realize(player, SL_BOOLEAN_FALSE);
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to realize OpenSLES audio player");
        return(0);
    } else {
//        SNDDBG2("Realize OpenSLES audio player - OK");
    }

    /* Get the play interface which is implicit */
    result = (*player)->GetInterface(player, SL_IID_PLAY, (void*)&playItf);
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to get OpenSLES audio player interface");
        return(0);
    } else {
//        SNDDBG2("Get OpenSLES audio player interface - OK");
    }

    /* Get the buffer queue interface which was explicitly requested */
    result = (*player)->GetInterface(player, SL_IID_ANDROIDSIMPLEBUFFERQUEUE, (void*)&decBuffQueueItf);
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to get OpenSLES decoder buffer queue interface");
        return(0);
    } else {
//        SNDDBG2("Get OpenSLES decoder buffer queue interface - OK");
    }
    ...
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to get OpenSLES prefetch interface");
        return(0);
    } else {
//        SNDDBG2("Get OpenSLES prefetch interface - OK");
    }

    /* Get the seek interface which was explicitly requested */
    result = (*player)->GetInterface(player, SL_IID_SEEK, (void*)&seekItf);
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to get OpenSLES seek interface\n");
        return(0);
    } else {
//        SNDDBG2("Get OpenSLES seek interface - OK\n");
    }
    ...
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to get OpenSLES MetadataExtraction interface\n");
        return(0);
    } else {
//        SNDDBG2("Get OpenSLES MetadataExtraction interface - OK\n");
    }
    ...
    file_container->dstData = file_container->dstDataBase;
//    SNDDBG2("Enqueueing initial empty buffers to receive decoded PCM data");
    ...
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to register OpenSLES decode buffer callback");
        return(0);
    } else {
//        SNDDBG2("Register OpenSLES decode buffer callback - OK");
    }
    ...
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to register OpenSLES prefetch event callback");
        return(0);
    } else {
//        SNDDBG2("Register OpenSLES prefetch event callback - OK");
    }

    result = (*prefetchItf)->SetCallbackEventsMask(prefetchItf, PREFETCHEVENT_ERROR_CANDIDATE);
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to set OpenSLES callback event mask for prefetch");
        return(0);
    } else {
//        SNDDBG2("Set OpenSLES callback event mask for prefetch - OK");
    }

    // set the player's state to paused, to start prefetching
//    SNDDBG2("Setting play state to PAUSED");
...
//    SNDDBG2("Awaiting prefetch complete");
    pthread_mutex_lock(&file_container->prefetch_mutex);
    ...
    if (file_container->prefetch_status == PREFETCHSTATUS_ERROR) {
        SNDDBG2("Error during prefetch, exiting");
        return(0);
    }
//    SNDDBG2("Prefetch is complete");
    ...
    if (file_container->channelCountKeyIndex == -1) {
        SNDDBG2("Unable to find key %s", ANDROID_KEY_PCMFORMAT_NUMCHANNELS);
    }
    if (file_container->sampleRateKeyIndex == -1) {
        SNDDBG2("Unable to find key %s", ANDROID_KEY_PCMFORMAT_SAMPLERATE);
    }

    /* ------------------------------------------------------ */
    /* Start decoding */
//    SNDDBG2("Starting to decode");
    result = (*playItf)->SetPlayState(playItf, SL_PLAYSTATE_PLAYING);
    if (SL_RESULT_SUCCESS != result) {
        SNDDBG2("Failed to set OpenSLES play state to SL_PLAYSTATE_PLAYING");
        return(0);
    }

//    SNDDBG2("Awaiting metadata before exiting OpenSLES_open()");
    pthread_mutex_lock(&file_container->decoder_mutex);
    while(file_container->available == SL_BOOLEAN_FALSE && file_container->eos == SL_BOOLEAN_FALSE) {
        pthread_cond_wait(&file_container->decoder_cond, &file_container->decoder_mutex);
    }
    pthread_mutex_unlock(&file_container->decoder_mutex);
//    SNDDBG2("Exiting OpenSLES_open()");
...
//        SNDDBG2("[EOS] total time=%d", internal->total_time);
...
//    SNDDBG2("OpenSLES_read %d bytes", BUFFER_SIZE_IN_BYTES);
...
//    SNDDBG2("OpenSLES_close calling Destroy in thread");
	(*file_container->player)->Destroy(file_container->player);
//    SNDDBG2("OpenSLES_close finished calling Destroy in thread");
...
{ 
	SNDDBG2("DestroyWorkaroundThreadExitHandler signal is %d. Calling pthread_exit(0).", sig);
	pthread_exit(0);
}
...
//    SNDDBG2("OpenSLES_close begin");
    ...
    pthread_mutex_lock(&file_container->decoder_mutex);
    // Waiting for decoder
//    SNDDBG2("OpenSLES_close Waiting for decoder");
    ...
    if (internal->decoder_private != NULL) {
        if (file_container->player != NULL) {
            ...
            #if SOUNDDECODER_OPENSLES_ANDROID_USE_DESTROY_DEADLOCK_WORKAROUND
//			    SNDDBG2("OpenSLES_close using SOUNDDECODER_OPENSLES_ANDROID_USE_DESTROY_DEADLOCK_WORKAROUND");
			    ...
			    while(!file_container->destroySucceeded && destroy_workaround_timeval.tv_sec < expire_time) {
				    ...
				    usleep(1000*10);
				    gettimeofday(&destroy_workaround_timeval, NULL);		
				    // SNDDBG2("spinning: flag=%d, tv2.tv_sec=%d, destroy_timeout.tv_sec=%d", my_destroy_flag, tv2.tv_sec, destroy_timeout.tv_sec);
			    }
			    ...
			    if(!file_container->destroySucceeded) {
			        ...
			        if ( (kill_status = pthread_kill(destroy_workaround_thread, SIGUSR1)) != 0) { 
			            SNDDBG2("Error cancelling Destroy() thread %d, error = %d (%s)", destroy_workaround_thread, kill_status, strerror(kill_status));
			        } 
			    }
			    else {
...
//    SNDDBG2("OpenSLES_close end");
}

static int OpenSLES_rewind(Sound_Sample *sample) {
//    SNDDBG2("OpenSLES_rewind");
    Sound_SampleInternal *internal = (Sound_SampleInternal *)sample->opaque;
    OpenSLESFileContainer *file_container = (OpenSLESFileContainer *)internal->decoder_private;
