...
static int MPG123_open(Sound_Sample *sample, const char *ext)
{
	...
	{
		SNDDBG2("mpg123 new failed");
        goto mpg123_open_failed;
	}
    else if ((rc = set_formats(mp)) != MPG123_OK)
	{
		SNDDBG2("mpg123 set_formats failed");
        goto mpg123_open_failed;
	}
    else if ((rc = mpg123_replace_reader(mp, rwread, rwseek)) != MPG123_OK)
	{

		SNDDBG2("mpg123 replace reader");
        goto mpg123_open_failed;
	}
    else if ((rc = mpg123_open_fd(mp, rwops)) != MPG123_OK)
	{
		SNDDBG2("mpg123 open_fd failed");
        goto mpg123_open_failed;
	}
	/* According to the FIXME below, this check might be wrong.
	 	I had a questionable build of mpg123 on Android armv7 which failed the scan, but managed to play. 
		(Turns out I needed to set 32-bits instead of 64-bit which broke rewinding and scan.)
		But it kind of worked, so I'm not sure if this check is useful.
	*/
#if 1
    else if ((rc = mpg123_scan(mp)) != MPG123_OK)
	{
		SNDDBG2("mpg123 scan failed");
        goto mpg123_open_failed;  /* !!! FIXME: this may be wrong. */
	}
#endif
    else if ((rc = mpg123_getformat(mp, &rate, &channels, &fmt)) != MPG123_OK)
	{
		SNDDBG2("mpg123 getformat failed");
        goto mpg123_open_failed;
	}
    ...
    */
	...
	{
		SNDDBG2("mpg123 format type failed");
        goto mpg123_open_failed;
	}
...
mpg123_open_failed:
	SNDDBG2("mpg123 goto open failed");
    SDL_LockMutex(mpg123_mutex);
