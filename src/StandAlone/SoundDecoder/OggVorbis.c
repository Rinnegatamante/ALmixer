...
static int OggVorbis_open(Sound_Sample* sound_sample, const char* file_ext)
{
	...
	*/
	{
		double total_time_sec = ov_time_total(file_ov, -1);
		if(OV_EINVAL == total_time_sec)
		{
			sample_internal->total_time = -1;
		}
		else
		{
			sample_internal->total_time = (ptrdiff_t)(total_time_sec * 1000.0 + 0.5);
		}
	}
	...
	SNDDBG2("OggVorbis: channels = %d\n", sound_sample->actual.channels);
	SNDDBG2("OggVorbis: sampling rate = %d\n",sound_sample->actual.rate);
	SNDDBG2("OggVorbis: total seconds of sample = %d\n", sample_internal->total_time);
	SNDDBG2("OggVorbis: sound_sample->actual.format = %d\n", sound_sample->actual.format);
...
//	SNDDBG2("ov_read start\n");
	...
	*/
	...
	while(free_bytes_remaining_to_fill > 0 && !(sound_sample->flags & SOUND_SAMPLEFLAG_EOF))
	{
		...
		actual_bytes_read = ov_read(ogg_vorbis_file_container->oggVorbisFile, buffer_start_point, free_bytes_remaining_to_fill, is_bigendian, bytes_per_sample, is_signed, &out_bitstream);
//		SNDDBG2("ov_read bytes_read: %d\n", actual_bytes_read);		
		if(OV_EBADLINK == actual_bytes_read)
		{
			SNDDBG2("Error: ov_read OV_EBADLINK\n");
		...
		{
			SNDDBG2("Error: ov_read OV_EINVAL\n");
		...
		else if(OV_HOLE == actual_bytes_read)
		{
			...
			{
				/* give up */
				SNDDBG2("Error: ov_read OV_HOLE. Was unable to recover.\n");
				SoundDecoder_SetError("Error: ov_read OV_HOLE. Was unable to recover.");
				sound_sample->flags |= SOUND_SAMPLEFLAG_ERROR;
			}
			else
			{
				SNDDBG2("Error: ov_read OV_HOLE, trying to recover\n");
				loop_counter++;
				continue;
			}
		...
		{
			SNDDBG2("Error: ov_read unexpected error\n");
		...
		else if(0 == actual_bytes_read)
		{
			...
			*/
			if(loop_counter < 1)
			{
//				SNDDBG2("ov_read 0: EOF\n");
				sound_sample->flags |= SOUND_SAMPLEFLAG_EOF;
			}
			else
			{
//				SNDDBG2("ov_read 0: EAGAIN\n");
...
static int OggVorbis_seek(Sound_Sample* sound_sample, size_t ms)
{
	Sound_SampleInternal* internal_sample = (Sound_SampleInternal*)sound_sample->opaque;
	OggVorbisFileContainer* ogg_vorbis_file_container = (OggVorbisFileContainer *) internal_sample->decoder_private;
    double time_pos_in_sec = (((double)ms) / 1000.0);
	int ret_val = ov_time_seek(ogg_vorbis_file_container->oggVorbisFile, time_pos_in_sec);
	if(OV_ENOSEEK == ret_val)
	{
		SNDDBG2("Error: ov_time_seek failed because bitstream is not seekable\n");
	...
	{
		SNDDBG2("Error: ov_time_seek failed\n");
